...
final class TestsModelRepository: XCTestCase {
    var localStorage: ModelRepositoryProtocol!

    func createLocalStorage() async throws -> ModelRepositoryProtocol { ModelRepository() }
    
    let firstBrandId = UUID()
    let secondBrandId = UUID()

    func createValue() -> ModelDTO { ModelDTO(brandId: firstBrandId, id: .init(), name: " ") }
    func createSecondValue() -> ModelDTO { ModelDTO(brandId: firstBrandId, id: .init(), name: " ") }
    func createThirdValue() -> ModelDTO { ModelDTO(brandId: secondBrandId, id: .init(), name: " ") }

    public override func setUp() async throws {
        try await super.setUp()
        let modelStorage = try await ModelNewStorageRealm()
        Injection.shared.register(ModelStorageProtocol.self) { _ in
            modelStorage
        }
    ...
    func test_GetByBrandId_When_No_Id_in_DB_expect_throw() async throws {
        let value = createValue()
    
        let result = try? await localStorage.get(byBrandId: value.brandId)

        XCTAssertEqual(result, [])
    }

    func test_GetByBrandId_When_Id_in_DB_expect_find_value() async throws {
        let value = createValue()
        let secondValue = createSecondValue()
        
        XCTAssertEqual(value.brandId, secondValue.brandId)
