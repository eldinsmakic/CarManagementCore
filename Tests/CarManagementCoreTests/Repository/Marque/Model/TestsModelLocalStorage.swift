...
final class TestsModelLocalStorage: XCTestCase {
    var localStorage: ModelStorageProtocol!

    func createLocalStorage() async throws -> ModelStorageProtocol { try await ModelNewStorageRealm() }
    ...
    func test_GetByBradId_When_Id_in_DB_expect_find_values() async throws {
        let value = createValue()
        let secondValue = createSecondValue()
        let thirdValue = createThirdValue()
        
        XCTAssertEqual(firstBrandId, value.brandId)
        XCTAssertEqual(value.brandId, secondValue.brandId)
        XCTAssertNotEqual(value.brandId, thirdValue.brandId)
        
        _ = try await localStorage.add(value)
        _ = try await localStorage.add(secondValue)
        _ = try await localStorage.add(thirdValue)
    
        let result = try? await localStorage.get(byBrandId: firstBrandId)
